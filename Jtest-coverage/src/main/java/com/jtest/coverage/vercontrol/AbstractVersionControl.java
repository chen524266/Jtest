...
import com.jtest.coverage.vercontrol.dto.*;
import com.jtest.coverage.vercontrol.enums.CodeManageTypeEnum;
import com.jtest.coverage.vercontrol.util.MethodParserUtils;
import com.jtest.coverage.vercontrol.util.XmlDiffUtils;
import lombok.Data;
...
public abstract class AbstractVersionControl {
    ...
    private CompletableFuture<DiffClassInfoResult> getClassMethods(String oldClassFile, String mewClassFile, DiffEntryDto diffEntry) {
        ...
        return CompletableFuture.supplyAsync(() -> {
            ...
            String className;
            if (diffEntry.getNewPath().contains(versionControlDto.getRootPath())) {
                className = diffEntry.getNewPath().split(versionControlDto.getRootPath())[1].split("\\.")[0];
            } else {
                className = "";
            }
            //新增类直接标记，不用计算方法
...
//            }
            ...
            //获取新类的所有方法
            List<MethodInfoResult> newMethodInfoResults = MethodParserUtils.parseMethods(mewClassFile, versionControlDto.getRootPath());
            //如果新类为空，没必要比较
            ...
            if (DiffEntry.ChangeType.MODIFY.equals(diffEntry.getChangeType())) {
                oldMethodInfoResults = MethodParserUtils.parseMethods(oldClassFile, versionControlDto.getRootPath());
            }
            //如果旧类为空，新类的方法所有为增量
