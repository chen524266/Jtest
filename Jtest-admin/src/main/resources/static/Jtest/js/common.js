...
var refreshItem = function(){
    ...
    var target = $('.Jtest_iframe[data-id="' + currentId + '"]', topWindow);
...
var closeItem = function(dataId){
	...
	if ($.common.isNotEmpty(dataId)) {
	    ...
	    // 移除相应tab对应的内容区
	    $('.mainContent .Jtest_iframe[data-id="' + dataId + '"]', topWindow).remove();
	    return;
	}
	...
	if ($.common.isNotEmpty(panelUrl)) {
	    ...
	    $('.mainContent .Jtest_iframe', topWindow).each(function() {
...
function createMenuItem(dataUrl, menuName, isRefresh) {
    ...
    if (flag) {
        ...
        // 添加选项卡对应的iframe
        var str1 = '<iframe class="Jtest_iframe" name="iframe' + dataIndex + '" width="100%" height="100%" src="' + dataUrl + '" frameborder="0" data-id="' + dataUrl + '" data-panel="' + panelUrl + '" seamless></iframe>';
        if (isScrollToTop) {
            $('.mainContent', topWindow).find('iframe.Jtest_iframe').hide().parents('.mainContent').append(str1);
        } else {
            $('.mainContent', topWindow).find('iframe.Jtest_iframe').css({"visibility": "hidden", "position": "absolute"}).parents('.mainContent').append(str1);
        }
...
function activeWindow() {
    ...
    return $('.Jtest_iframe[data-id="' + currentId + '"]', topWindow)[0].contentWindow;
}
