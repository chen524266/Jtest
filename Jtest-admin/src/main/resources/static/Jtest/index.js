...
function openToCurrentTab(obj) {
    if (isScrollToTop) {
        $(obj).show().siblings('.Jtest_iframe').hide();
    } else {
        $(obj).css({"visibility": "visible", "position": "static"}).siblings('.Jtest_iframe').css({"visibility": "hidden", "position": "absolute"});
    }
}
...
$(function() {
    ...
    function menuItem() {
        ...
        $('.menuTab').each(function() {
            if ($(this).data('id') == dataUrl) {
                if (!$(this).hasClass('active')) {
                    ...
                    scrollToTab(this);
                    // 显示tab对应的内容区
                    $('.mainContent .Jtest_iframe').each(function() {
                        if ($(this).data('id') == dataUrl) {
                            openToCurrentTab(this);
                            return false;
                        }
        ...
        if (flag) {
            ...
            // 添加选项卡对应的iframe
            var str1 = '<iframe class="Jtest_iframe" name="iframe' + dataIndex + '" width="100%" height="100%" src="' + dataUrl + '" frameborder="0" data-id="' + dataUrl + '" data-refresh="' + isRefresh + '" seamless></iframe>';
            if (isScrollToTop) {
                $('.mainContent').find('iframe.Jtest_iframe').hide().parents('.mainContent').append(str1);
            } else {
                $('.mainContent').find('iframe.Jtest_iframe').css({"visibility": "hidden", "position": "absolute"}).parents('.mainContent').append(str1);
            }
    ...
    function menuBlank() {
    	// 新窗口打开外网以http://开头，如http://Jtest.vip
    	var dataUrl = $(this).attr('href');
    	window.open(dataUrl);
    	return false;
    }
    ...
    function refreshTab() {
    	...
    	var target = $('.Jtest_iframe[data-id="' + currentId + '"]');
    ...
    function tabCloseOther() {
        $('.page-tabs-content').children("[data-id]").not(":first").not(".active").each(function() {
            $('.Jtest_iframe[data-id="' + $(this).data('id') + '"]').remove();
            $(this).remove();
        });
    ...
    function tabCloseAll() {
        ...
        $('.page-tabs-content').children("[data-id]:first").each(function() {
            if (isScrollToTop) {
                $('.Jtest_iframe[data-id="' + $(this).data('id') + '"]').show();
            } else {
                $('.Jtest_iframe[data-id="' + $(this).data('id') + '"]').css({"visibility": "visible", "position": "static"});
            }
    ...
    $.contextMenu({
        ...
        items: {
            ...
            "full": {
                ...
                callback: function(key, opt) {
                    setActiveTab(this);
                    var target = $('.Jtest_iframe[data-id="' + this.data('id') + '"]');
                    target.fullScreen(true);
                }
            },
            "refresh": {
